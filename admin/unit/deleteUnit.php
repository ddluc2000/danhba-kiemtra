<?php
    ...
    $sql = "DELETE FROM tbdonvi WHERE id = $id_can_xoa";
    $result = mysqli_query($conn,$sql);

    //Bước 03: Xử lý kết quả nếu mysqli_query xóa thành công > trả về true
    if($result == true){
        $_SESSION['delete'] = "<div class='success'>Deleted Unit Successfully.</div>";
        header('location:'.SITEURL.'/admin/unit/unitManagement.php');
    }
    else{
        $_SESSION['delete-mes'] = "<div class='success'>Deleted Unit Failed.</div>";
        header('location:'.SITEURL.'/admin/unit/unitManagement.php');
    }
